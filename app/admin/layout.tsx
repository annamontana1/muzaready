...
const menuItems = [
  ...
  { href: '/admin/warehouse-scanner', icon: 'M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z', label: 'Scanner' },
  ...
  { href: '/admin/coupons', icon: 'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z', label: 'Kupóny' },
  ...
  { href: '/admin/seo', icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z', label: 'SEO' },
  { href: '/admin/content', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z', label: 'Obsah' },
  { href: '/admin/redirects', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1', label: 'Redirecty' },
    ...
    <QueryClientProvider client={queryClient}>
      <ToastProvider>
        <div className="flex h-screen bg-slate-50 overflow-hidden">
          ...
          >
            {/* Logo Header */}
            <div className="h-14 lg:h-16 flex items-center justify-between px-4 border-b border-slate-700/50 flex-shrink-0">
              {sidebarOpen && (
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
                    <span className="text-white font-bold text-sm">M</span>
                  </div>
                  <span className="font-semibold text-base whitespace-nowrap">Mùza Admin</span>
                </div>
              )}
              <button
            ...
            {/* Navigation */}
            <nav className="flex-1 py-2 lg:py-4 px-2 lg:px-3 space-y-0.5 lg:space-y-1 overflow-y-auto">
              {menuItems.map((item, index) => {
                if (item.divider) {
                  return sidebarOpen ? (
                    <div key={index} className="pt-3 lg:pt-4 pb-1 lg:pb-2">
                      <span className="px-3 text-[10px] lg:text-xs font-semibold text-slate-500 uppercase tracking-wider">
                        {item.label}
                      </span>
                    </div>
                  ) : (
                    <div key={index} className="py-1 lg:py-2 hidden lg:block">
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href!}
                    className={`flex items-center gap-2 lg:gap-3 px-3 py-2 lg:py-2.5 rounded-lg transition-all duration-200 group ${
                      isActive
            ...
            {/* Bottom Actions */}
            <div className="p-2 lg:p-3 border-t border-slate-700/50 space-y-1 lg:space-y-2 flex-shrink-0">
              <Link
                href="/"
                className={`flex items-center gap-2 lg:gap-3 px-3 py-2 lg:py-2.5 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-all ${!sidebarOpen ? 'justify-center' : ''}`}
                title={!sidebarOpen ? 'Zpět na e-shop' : undefined}
              >
                <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
                {sidebarOpen && <span className="text-sm font-medium">E-shop</span>}
              </Link>
              <button
                onClick={handleLogout}
                className={`w-full flex items-center gap-2 lg:gap-3 px-3 py-2 lg:py-2.5 rounded-lg bg-red-600/10 text-red-400 hover:bg-red-600 hover:text-white transition-all ${!sidebarOpen ? 'justify-center' : ''}`}
                title={!sidebarOpen ? 'Odhlásit se' : undefined}
              >
                <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
                {sidebarOpen && <span className="text-sm font-medium">Odhlásit</span>}
              </button>
            </div>
          </aside>
          ...
          <main className="flex-1 overflow-auto min-w-0">
            {/* Top Bar */}
            <div className="h-14 lg:h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-10">
              <div className="flex items-center gap-3 lg:gap-4">
                {/* Mobile menu button */}
                <button
                  onClick={() => setSidebarOpen(true)}
                  className="p-2 -ml-2 lg:hidden hover:bg-slate-100 rounded-lg"
                >
