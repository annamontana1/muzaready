    ...
    return (
      <div className="min-h-screen bg-soft-cream flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-text-mid">Načítám potvrzení objednávky...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-soft-cream">
        <div className="max-w-2xl mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <h1 className="text-2xl font-bold text-text-dark mb-4">Chyba</h1>
            <p className="text-text-mid mb-6">{error || 'Objednávka nebyla nalezena'}</p>
            <Link
              href="/"
    ...
    <div className="min-h-screen bg-soft-cream">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <h2 className="text-xl font-bold text-text-dark mb-6">Podrobnosti objednávky</h2>

          {/* Order ID and Status */}
          <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b border-warm-beige">
            <div>
              <p className="text-sm text-text-mid">Číslo objednávky</p>
              <p className="text-lg font-semibold text-text-dark">{order.id}</p>
            </div>
            <div>
              <p className="text-sm text-text-mid">Stav</p>
          ...
          {/* Items */}
          <div className="mb-6 pb-6 border-b border-warm-beige">
            <h3 className="font-semibold text-text-dark mb-4">Položky v objednávce</h3>
            <div className="space-y-4">
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between items-start">
                  <div>
                    <p className="font-medium text-text-dark">{item.nameSnapshot || item.skuId}</p>
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <p className="text-text-mid">Mezisoučet:</p>
              <p className="font-medium text-text-dark">{subtotal.toLocaleString('cs-CZ')} Kč</p>
            </div>
            <div className="flex justify-between">
              <p className="text-text-mid">Doprava:</p>
            ...
            {discount > 0 && (
              <div className="flex justify-between">
                <p className="text-text-mid">Sleva:</p>
                <p className="font-medium text-green-600">-{discount.toLocaleString('cs-CZ')} Kč</p>
              </div>
            )}
            <div className="border-t border-warm-beige pt-3 flex justify-between">
              <p className="text-lg font-bold text-text-dark">Celkem:</p>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
          <h3 className="font-semibold text-text-dark mb-3">Co bude dál?</h3>
          <ol className="list-decimal list-inside space-y-2 text-text-mid text-sm">
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-soft-cream flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-text-mid">Načítám...</p>
        </div>
      </div>
    }>
      <ConfirmationContent />
